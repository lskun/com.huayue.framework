...
public class Base64
{
    ...
    {
        byte[] btcl = new byte[bt.length + (bt.length % 3 == 0 ? 0 : (bt.length % 3 == 1 ? 2 : 1))];// 补齐为3的倍数的数据源集合
        ...
        {
            // 不做折行处理
            int n = ((btcl[c] << 16) & 0xff0000) | ((btcl[c + 1] << 8) & 0xff00) | (btcl[c + 2] & 0xff);
    ...
    public static byte[] decodeBase64(byte[] bt)
    {
        ...
        {
            int n = (base64_inv[bt[c]] << 18) | (base64_inv[bt[c + 1]] << 12) | (base64_inv[bt[c + 2]] << 6)
                    | base64_inv[bt[c + 3]];
            r[j] = (byte) (n >>> 16 & 0xff);
            r[j + 1] = (byte) (n >>> 8 & 0xff);
            r[j + 2] = (byte) (n & 0xff);
    ...
    {
    	String aa="zvcvzcxvzcxvzcxvzcxv";
    	
    	System.out.println(Base64.encodeBase64(aa.getBytes()).toString());
    }
