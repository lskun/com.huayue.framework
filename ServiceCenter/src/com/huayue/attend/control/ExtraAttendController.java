...
public class ExtraAttendController {
	...
	public String addExtraForm(UserSpeday userSpeday, HttpServletRequest request ,ModelMap model){
		...
		try{
			...
			if(!StringUtils.isBlank(request.getParameter("out_Time"))){
				userSpeday.setOutTime(Double.parseDouble(request.getParameter("out_Time")));
			}else{
				userSpeday.setOutTime(0);
			}
	...
	public String showExact(ModelMap model,HttpServletRequest request){
		...
		String eTime = request.getParameter("endTime") == null ? DateUtil.dateToString(new Date(System.currentTimeMillis()), DateUtil.LONG_DATE_FORMAT) + " 23:59:00" : request.getParameter("endTime");
	...
	public String computeWorkOverTime(ModelMap model ,HttpServletRequest request){
		...
		try{
			...
			sTime = request.getParameter("startTime") == null ? DateUtil.getMonthFirstDay() + " 00:00:00" : request.getParameter("startTime");
			eTime = request.getParameter("endTime") == null ? DateUtil.dateToString(new Date(System.currentTimeMillis()), DateUtil.LONG_DATE_FORMAT) + " 23:59:00" : request.getParameter("endTime");
	...
	@RequestMapping(value="totalOverWork")
	public String computeTotalOverTime(ModelMap model ,HttpServletRequest request){
		...
		try{
			if(
					((User)session.getAttribute("login_user")).getPermission() == 0){
				return "permission_deny";
			}
			sTime = request.getParameter("sTime") == null ? DateUtil.getMonthFirstDay() + " 00:00:00" : request.getParameter("sTime");
			eTime = request.getParameter("eTime") == null ? DateUtil.dateToString(new Date(System.currentTimeMillis()), DateUtil.LONG_DATE_FORMAT) + " 23:59:00" : request.getParameter("eTime");
	...
	@RequestMapping("showOverWorkDetail")
	public @ResponseBody List<OverTimeInfo> showOverTimeDetail(@RequestParam("id")int id,HttpServletRequest request){
		List<OverTimeInfo> list = null;
		String sTime = request.getParameter("sTime") == null ? DateUtil.getMonthFirstDay() + " 00:00:00" : request.getParameter("sTime");
		String eTime = request.getParameter("eTime") == null ? DateUtil.dateToString(new Date(System.currentTimeMillis()), DateUtil.LONG_DATE_FORMAT) + " 23:59:00" : request.getParameter("eTime");
		try {
			list = userSpedayDao.showOverTimeList(id, sTime, eTime);
		} catch (Exception e) {
