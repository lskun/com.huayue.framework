...
public class UserSpedayDaoImpl implements UserSpedayDao{
	...
	public void add(final UserSpeday speday) {
		...
		String sql = "INSERT INTO USER_SPEDAY_TEMP (USERID ,STARTTIME ,ENDTIME ,DATAID ,REASON ,DATE ,OUTTIME) VALUES (?,?,?,?,?,?,?)";
		jdbcTemplate.update(sql, new PreparedStatementSetter(){
			public void setValues(PreparedStatement rs) throws SQLException {
				...
				rs.setDouble(7, speday.getOutTime());
			}
		}				
		);
	}
	...
	public List<OverTimeInfo> showOverTimeList(final long userId,String sTime,String endTime)throws Exception{
		...
		String sql = "SELECT TID, STARTTIME, ENDTIME, REASON, DATE, CHECKID,TOTALTIME FROM USER_SPEDAY_TEMP WHERE USERID = ? AND ( STARTTIME > ? AND ENDTIME < ? ) AND STATUS = 1 AND DATAID = 1005";
	...
	public TreeMap<Integer,Long> showTotalOverTimeList(String sTime,String eTime)throws Exception{	
		...
		String sql = "SELECT USERID, TOTALTIME FROM USER_SPEDAY_TEMP WHERE STARTTIME > ? AND ENDTIME < ? AND STATUS = 1 AND DATAID = 1005";
		...
		for(int i = 0 ,j = list.size(); i < j ; i++ ){
			...
			double time = (Double)map.get("TOTALTIME")* 60;
			...
			long temp = (long) (total.get(id) + time);
					//(c2.getTimeInMillis() - c1.getTimeInMillis()) / (1000 * 60);
			total.put(id, temp); 
		}
