...
public class LibraryCategoryAct {
	...
	public void addCategory(Category category,ModelMap model,HttpServletRequest request,HttpServletResponse response) throws IOException{
		...
		try{
			...
			mappings.put(category.getId(), category.getHierarchical_relation());
	...
	public String delCategory(HttpServletRequest request ,ModelMap model) throws IOException{
		...
		libraryCategoryService.batchDelete(ids);
		model.addAttribute(Constants.RESULT, Constants.DELETE_SUCCESS_TEXT);
		model.addAttribute("url", "v_list.do");
		return "feedback";
	}	
